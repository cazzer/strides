...
export function computeStepWidth(
  ...
  for (const candidate of candidates) {
    ...
    // the foot landed. Same bucket as the `continue`s below — an ankle that failed to resolve while
  ...
  // candidate footstrikes this metric could measure — a resolvable hip position at that instant AND
  // an ankle pair that still names two feet (`ankleMeasurable`).
  ...
  if (usableStrikeCount < MIN_STEP_WIDTH_SAMPLE_SIZE) {
    caveats.push(
      ...
      `Only ${usableStrikeCount} of ${candidates.length} detected footstrike(s) were usable (recommend at least ${MIN_STEP_WIDTH_SAMPLE_SIZE}) — confidence reduced accordingly.`,
    )
  }
  if (value < 0) {
    caveats.push(
