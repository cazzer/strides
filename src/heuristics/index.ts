...
/**
 * View detection must run before all eight metrics — each one's view-fit gating and confidence
 * depend on the detected `View`. Encoding that ordering dependency here, rather than leaving it
 * to every caller, means the results view makes one call instead of nine that have to be
