 ...
 *
 * ### One detached decoder at a time, process-wide — the same one evidence extraction uses
 *
 ...
 * analogue here. The ordering therefore lives INSIDE the derivation, on `videoElement.ts`'s
 * `queueDetachedDecode`: a second call cannot start its decoder until the first has torn its own
 * down, whoever makes it and whenever. That is the same "sequential by construction rather than by
 * caller convention" posture and the same reason — Demo 1 is 3840x2160 and Demo 2 is 2160x3840, and
 * a multi-file pick mounts every clip in one tick, so N concurrent 4K decoders is not an acceptable
 * amount of memory to hold, on top of the WebGL sampling run they would compete with.
 *
 * That queue is ONE queue for the whole app rather than one per feature. Posters and evidence images
 * each kept a tail of their own until `videoElement.ts` existed, which bounded each KIND at one
 * decoder and the pair at two; they are now ordered against each other as well, so the ceiling is
 * one decoder globally. The separation that kept the two apart in practice was timing, not design:
 * posters derive at clip-add time and evidence extraction only starts after analysis, and nothing
 * held those windows apart.
 *
 * ### No serialization
 *
 ...
 * 2D context. A poster is optional everywhere it is consumed, so every failure this function
 * DECIDES is a `null` rather than a throw — which is not the same as never throwing, and the doc
 * here used to overstate it as if it were. See `deriveClipPoster` for what happens to the ones it
 * does not decide.
 */
 ...
 * teardown are `withDecodedVideo`'s (`videoElement.ts`) — identical plumbing to the evidence path's,
 * because it is literally the same code rather than a second copy of it. An element that never
 * decodes yields no poster, the same `null` every other failure yields.
 */
function decodePosterFrame(
  sourceBlob: Blob,
  metadata: VideoMetadata,
  options: PosterDerivationOptions,
): Promise<ClipPoster | null> {
  return withDecodedVideo(
    ...
    (video) => drawPosterFrame(video, metadata, options),
    () => null,
  )
}
 ...
 *
 * "Already asked for" spans evidence extraction too, not just other posters: the wait is
 * `videoElement.ts`'s single `queueDetachedDecode`, shared with `extractClipEvidence`. It is
 * imposed here rather than asked of callers because `useClipPoster` is mounted once per clip and
 * cannot see the other clips, so a picked-4-files session fires four of these in a single microtask
 * flush with nothing between them. Chaining onto one module-level tail makes "one detached decoder
 * exists at a time" a property of the shared module instead of a convention every future call site
 * has to know about — the same guarantee `extractSessionEvidence` gets from its `for await` loop,
 * reached the only way an N-caller API can reach it.
 *
 ...
 *
 * ### It resolves `null` on every failure it knows about, and rejects on the ones it does not
 *
 * Undecidable size, no reachable instant, no 2D context, an element that never decodes: all `null`.
 * A poster is optional everywhere it is consumed, so none of those is worth a throw. But this is
 * NOT a blanket never-throws contract — an unforeseen browser failure (`URL.createObjectURL`
 * refusing, `drawImage` on a decoder in an invalid state) propagates and this promise rejects.
 *
 * That is deliberate rather than an oversight, and it is why the mismatched "every failure is a
 * `null`, never a throw" claim was corrected instead of a `catch` being added to satisfy it. Every
 * failure mode this code knows about is already a `null`, so a throw is by definition a bug;
 * swallowing it would leave the poster `null` either way while erasing the only signal that the bug
 * exists. `posterFrame.test.ts` pins both halves — that a failed derivation rejects, and that the
 * shared queue survives it and grants the next clip its turn.
 */
export function deriveClipPoster(
  ...
  return queueDetachedDecode(() =>
    decodePosterFrame(sourceBlob, metadata, options),
  )
}
