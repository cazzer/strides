...
function MetricCard({ metric, chart }: MetricCardProps) {
  ...
  return (
    <article
      className="metrics-panel__card"
      data-flagged={isFlagged}
      style={isFlagged ? { opacity: 0.85, borderColor: 'currentColor' } : undefined}
      aria-label={METRIC_LABELS[metric.metric]}
    >
      <h3>{METRIC_LABELS[metric.metric]}</h3>
      <p className="metrics-panel__value">{formatValue(metric)}</p>
      <p className="metrics-panel__description">{METRIC_DESCRIPTIONS[metric.metric]}</p>
      <p className="metrics-panel__confidence">
        <strong>{confidenceLabel(metric)}</strong>
        {metric.viewFit === 'unsuitable' && ' — not reliable from this camera angle'}
      </p>
      {metric.caveat && (
        <p role="note" className="metrics-panel__caveat">
          {metric.caveat}
        </p>
      )}
      {chart}
...
export function MetricsPanel({ heuristics }: MetricsPanelProps) {
  return (
    <section className="metrics-panel" aria-label="Form metrics">
      <MetricCard
