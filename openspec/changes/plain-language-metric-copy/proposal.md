...
- **Fit-quality caveats** (cadence, verticalOscillation, verticalOscillationCm, verticalRatio):
  the R²-with-threshold sentences become "the step/bounce rhythm in this clip was too irregular
  to read confidently — confidence reduced accordingly", and the below-gate null-value caveats
  become "…the step/bounce rhythm was too irregular to measure". Failure caveats still
  distinguish WHAT failed (too few frames vs. too short vs. no motion vs. irregular rhythm).
...
- **Scale-pass narrative** (results status line, excluded-entry hints, provenance sentence): no
  ...
  look at / second pass of the same clip.
...
- **Jargon in degraded-value caveats**: "tracked stretches" → "stretches of the clip",
  ...
  were left out of the measurement".
